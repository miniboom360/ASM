...
def export_all_results(path, name, fmt, datas):
    ...
    logger.log('ALERT', f'The subdomain result for all gobuster_service domains: {path}')
    row_list = list()
    for row in datas:
        if 'header' in row:
            row.pop('header')
...
def export_all_subdomains(alive, path, name, datas):
    ...
    logger.log('ALERT', f'The txt subdomain result for all gobuster_service domains: {path}')
    subdomains = set()
    for row in datas:
...
def match_subdomains(domain, html, distinct=True, fuzzy=True):
    ...
    :param  str domain: gobuster_service domain
