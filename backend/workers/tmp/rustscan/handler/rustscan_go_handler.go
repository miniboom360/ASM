...
const (
	rustscan_test_path = "/usr/local/Homebrew/bin/rustscan"
	rustscan_path      = "rustscan"
)

func RustScan(ip string) ([]byte, error) {
	uuid := uuid2.New()
	resultfile := fmt.Sprintf("%s.txt", uuid.String())
	command := fmt.Sprintf(rustscan_path + " -a " + ip + " --ulimit 5000 > " + resultfile)
	// out, err := exec.Command(rustscan_path, "-a", ip, "--ulimit",
	// 	"5000 >> ", resultfile).Output()
	...
	HandlerRustResult(resultfile, ip)
	return nil, nil
}

// handle result write in txt from rustscan
// --ulimit 5000
func HandlerRustResult(file_path, ip string) ([]byte, error) {
